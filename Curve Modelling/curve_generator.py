...
def generate_template(template_path, z, peak_semester):
    path = template_path
...
def generate_time_series(template, z, peak_semester):
    ...
    sn_flux_k = 4500 * (ld_known / ld) ** 2
    sn_flux_j = 1700 * (ld_known / ld) ** 2

    # Get the x and y arrays for the template, given a peak_semester and redshift
    dummy_curve_x, dummy_curve_y = generate_template(template, z, peak_semester)
